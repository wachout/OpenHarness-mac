...
def default_provider_profiles() -> dict[str, ProviderProfile]:
    ...
    return {
        ...
        "dashscope": ProviderProfile(
            ...
            allowed_models=["qwen-plus", "qwen3.5-plus", "qwen3.6-plus", "qwen3.5-flash", "qwen3-max", "deepseek-r1"],
        ),
        ...
        "stepfun": ProviderProfile(
            ...
            provider="stepfun",
            api_format="openai",
            auth_source="stepfun_api_key",
            default_model="step-2-mini",
            base_url="https://api.stepfun.com/v1",
        ),
        "baichuan": ProviderProfile(
            label="Baichuan AI (百川)",
            provider="baichuan",
            api_format="openai",
            auth_source="baichuan_api_key",
...
def _apply_env_overrides(settings: Settings) -> Settings:
    ...
    # Auto-detect DashScope when DASHSCOPE_API_KEY is set and no other key is found
    dashscope_key = os.environ.get("DASHSCOPE_API_KEY")
    if dashscope_key and not api_key:
        updates["api_key"] = dashscope_key
        if not os.environ.get("OPENHARNESS_API_FORMAT") and not updates.get("api_format"):
            updates.setdefault("api_format", "openai")
        if not os.environ.get("OPENHARNESS_PROVIDER") and not updates.get("provider"):
            updates.setdefault("provider", "dashscope")
        if not openharness_base and not profile_has_base_url and not updates.get("base_url"):
            updates.setdefault("base_url", "https://dashscope.aliyuncs.com/compatible-mode/v1")
        # Switch active profile to dashscope so materialize_active_profile() picks up correct settings
        if not os.environ.get("OPENHARNESS_PROVIDER"):
            _active_name = (settings.active_profile or "").strip() or "claude-api"
            _active_prof = settings.merged_profiles().get(_active_name)
            if _active_prof and _active_prof.auth_source in ("anthropic_api_key", "openai_api_key"):
                updates.setdefault("active_profile", "dashscope")
