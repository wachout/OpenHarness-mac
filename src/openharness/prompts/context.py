...
def build_runtime_system_prompt(
    ...
    *,
    ...
    # Workspace boundary notice: inform the model about read/write scope
    if workspace_root:
        root_str = str(Path(workspace_root).resolve())
        sections.append(
            "# Workspace Boundary\n"
            f"Your workspace directory is: {root_str}\n"
            "You have full read and write access to files within this workspace directory. "
            "Files outside this directory are read-only — you may read them but must not modify, "
            "delete, or create files outside the workspace. "
            "If you need to modify a file outside the workspace, inform the user and ask them to adjust the workspace boundary."
        )
